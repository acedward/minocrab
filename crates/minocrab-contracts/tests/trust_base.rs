...
const ROWS: &[Row] = &[
    ...
    Row { path: "minocrab-macros/src/ledger.rs", warrant: "the derive's unit tests; in_block pinned against `batch` for every block size (minocrab-std); the sixteen-field compactc probe; the headed layout through real blocks (v3_header, ledger_header) and the ZKIR dump (every block without a standard byte-identical to the base); `initial_state()`: the derive's unit tests (thinness, every field contributed in order) and the states it builds in v3_state and v3_stream", hides: "a ledger field laid out at a path compactc would not use, or a block given a headed layout it did not ask for, or a block whose `initial_state()` leaves a field out" },
    Row { path: "minocrab-macros/src/ledger_header.rs", warrant: "the derive's unit tests (expansion text, thinness, the fifteen-field bound, the rejected shapes); v3_header and ledger_header (the expansion's paths and the circuits built on them); the FR-005 compile-fail doctests; the standard's `InitialState` (the magic, sealed, then its fields): the unit test and v3_state (the magic at [0] / [0, 0], refused by `set`)", hides: "a header field placed at an entry other than [0, i + 1], or a compile-time check the expansion forgets to emit" },
    Row { path: "minocrab-macros/src/ledger_repr.rs", warrant: "v3_ledger `derived_repr` (atoms and limb round trip); the erc20_vault_pending lineage's slots; `default_stored` in field order: the unit test", hides: "an environment's limbs split at the wrong boundaries on read-back" },
