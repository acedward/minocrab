...
| `minocrab-ledger/src/lib.rs` | 105 | **READING** | nothing beyond the crate doc and mod/pub-use plumbing (102 lines) |
...
| `minocrab-ledger/src/ops.rs` | 1210 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
...
| `minocrab-std/src/v3.rs` | 2803 | every differential; v3_leaves / v3_bounded / v3_literals / v3_secp; lean_claims (the typed-leaf claims, crates/minocrab-std/lean). `from_field_unchecked` sites: **READING** (the grep in TRUST.md §3) | a leaf whose type promises a bound its constructor did not constrain |
| `minocrab-std/src/v3/ledger.rs` | 2352 | every contract differential; v3_ledger; nested_typed; the derive's layout pinned against compactc's `batch` for all 256 block sizes and the sixteen-field probe; the headed layout (`in_layout`) pinned against `batch` + 1 for every body size 0..=256 and its frozen fifteen-wide segment by literal paths | a typed slot reading the wrong field, or a segmented path computed differently from compactc — or a headed body path that collides with root[0] or moves when compactc's segment width does |
...
| `minocrab-macros/src/ledger.rs` | 544 | the derive's unit tests; in_block pinned against `batch` for every block size (minocrab-std); the sixteen-field compactc probe; the headed layout through real blocks (v3_header, ledger_header) and the ZKIR dump (every block without a standard byte-identical to the base) | a ledger field laid out at a path compactc would not use, or a block given a headed layout it did not ask for |
| `minocrab-macros/src/ledger_header.rs` | 335 | the derive's unit tests (expansion text, thinness, the fifteen-field bound, the rejected shapes); v3_header and ledger_header (the expansion's paths and the circuits built on them); the FR-005 compile-fail doctests | a header field placed at an entry other than [0, i + 1], or a compile-time check the expansion forgets to emit |
...
29780 lines in the seven crates; 11619 of them in files whose warrant is READING in whole or in part (the rows in bold).
<!-- GENERATED END -->
