...
| `minocrab-macros/src/lib.rs` | 343 | **READING** | nothing beyond wrappers (the expansions are the modules below) |
...
| `minocrab-macros/src/ledger.rs` | 610 | the derive's unit tests; in_block pinned against `batch` for every block size (minocrab-std); the sixteen-field compactc probe; the headed layout through real blocks (v3_header, ledger_header) and the ZKIR dump (every block without a standard byte-identical to the base); `initial_state()`: the derive's unit tests (thinness, every field contributed in order) and the states it builds in v3_state and ledger_deploy | a ledger field laid out at a path compactc would not use, or a block given a headed layout it did not ask for, or a block whose `initial_state()` leaves a field out |
| `minocrab-macros/src/ledger_header.rs` | 392 | the derive's unit tests (expansion text, thinness, the fifteen-field bound, the rejected shapes); v3_header and ledger_header (the expansion's paths and the circuits built on them); the FR-005 compile-fail doctests; the standard's `InitialState` (the magic, sealed, then its fields): the unit test and v3_state / ledger_deploy (the magic at [0] / [0, 0], bit-identical through every circuit) | a header field placed at an entry other than [0, i + 1], or a compile-time check the expansion forgets to emit |
...
30745 lines in the seven crates; 11809 of them in files whose warrant is READING in whole or in part (the rows in bold).
<!-- GENERATED END -->
