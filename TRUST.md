...
| `minocrab-ir/src/v3.rs` | 961 | every instruction-level differential (the emitted stream equals compactc's); v3_builder. The operand-type TABLES themselves: **READING** against zkir-v3/src/ir_vm.rs | an instruction emitted with an operand type the VM rejects, on a path no fixture takes |
...
| `minocrab/src/v3/disclose.rs` | 580 | the generated set-equality test on every circuit; disclosure_report; v3_disclose | a disclosure recorded under a label the signature does not name |
| `minocrab/src/v3/effects.rs` | 318 | v3_guard_scope; every differential with a branch | an effect escaping its guard |
| `minocrab-ledger/src/lib.rs` | 108 | **READING** | nothing beyond the crate doc and mod/pub-use plumbing (102 lines) |
| `minocrab-ledger/src/impact.rs` | 355 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
| `minocrab-ledger/src/ops.rs` | 1267 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code; the deploy-state values (`stored_cell`, `historic_merkle_tree_reset_value`): the crate's unit tests, v3_state (equal to the resets run through the VM) and ledger_deploy T4b (equal to compactc's generated initialState) | an Impact op encoded so the ledger applies a different state change than the circuit claims, or a deploy-state value other than the one the reset leaves |
| `minocrab-ledger/src/reads.rs` | 553 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
| `minocrab-ledger/src/kernel.rs` | 324 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
| `minocrab-ledger/src/calls.rs` | 306 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
| `minocrab-ledger/src/tests.rs` | 1149 | **READING** | a unit test that pins the wrong constant or asserts a weaker property than intended, unnoticed because it still passes |
| `minocrab-std/src/lib.rs` | 51 | **READING** | nothing beyond re-exports (51 lines) |
| `minocrab-std/src/v3.rs` | 2959 | every differential; v3_leaves / v3_bounded / v3_literals / v3_secp; lean_claims (the typed-leaf claims, crates/minocrab-std/lean). `from_field_unchecked` sites: **READING** (the grep in TRUST.md §3) | a leaf whose type promises a bound its constructor did not constrain |
| `minocrab-std/src/v3/ledger.rs` | 2593 | every contract differential; v3_ledger; nested_typed; the derive's layout pinned against compactc's `batch` for all 256 block sizes and the sixteen-field probe; `Placement::root_header` and `standards` (unit tests: a hand-written standard, the width and count refusals; the compile-fail doctests on v3::LedgerHeader); the headed layout (`in_layout`) pinned against `batch` + 1 for every body size 0..=256 and its frozen fifteen-wide segment by literal paths, and against compactc 0.34.0's own contract-info.json and ZKIR for magic-first twins of 4 and 14 own fields (header_small_differential, whose 15-own-field negative control pins where the two part ways); each slot type's initial value (`InitialState`, `LedgerRepr::default_stored`): v3_state (the builder's tree equals every field's reset run through the VM) and ledger_deploy T4b (byte-equal to compactc's JS, the curve points' identity defaults included, and inside a record: wrapped_points); the headed layout's cost: ledger_header's T8 probe (with and without a standard, the same (k, rows) and instruction count at every size from 3 to 256 own fields) | a typed slot reading the wrong field, or a segmented path computed differently from compactc — or a headed body path that collides with root[0], moves when compactc's segment width does, or sits a level deeper than compactc's (T8's rows would move), or a slot whose deploy-time value is not compactc's |
| `minocrab-std/src/v3/header.rs` | 248 | v3_header (real #[derive(Ledger)] blocks of 0-30 own fields against the same fields' compactc layout, the header at [0] / [0, 0] / [0, i + 1], declaration-order independence, header fields' ops byte-identical to hand-placed slots, a headed stream byte-identical to its compactc twin); ledger_header (Signet, Pending and Outbox in a headed block byte-identical to compactc twins; the notification's moved path on the ZKIR); header_small_differential (a magic-only standard with 4 and 14 own fields: every circuit's canonical ZKIR, the layout and the initial state equal to compactc 0.34.0's magic-first Compact twin); the compile-fail doctests on v3::LedgerHeader and signet_flow::Signet, one per FR-005 rejection, a standard written BY HAND included (a root placement with no LedgerHeader, a zero magic, a nonzero width, `implements` of a zero magic) and the derive without its impl; v3_state (a Magic only at [0] / [0, 0]; a standard hidden in a group slot, a missing or an extra magic refused by name). WHY each doctest fails (a compile_fail doctest passes on any error): **READING** | a rejection that lets two standards, a zero magic or a group slot through while its doctest still fails for another reason, or a magic, placement, width or one-standard rule that holds for the derive and not for a hand-written standard (the field rules are the derive's by design); a magic handle that exposes a write |
| `minocrab-std/src/v3/state.rs` | 380 | v3_state (the builder's tree equals the skeleton with every field's resetToDefault run through Midnight's VM, flat, segmented and headed; the header shapes; every refusal of `set`, the Array slots' shapes included, and of `magic`: a second magic, a standard hidden in a group slot, a headed block without its magic, a body slot that writes one); ledger_deploy (T4: every SC-003 size and the widest block, 256 own fields, deploys through the ledger's fee calculation, well_formed with its limits on, and apply, and circuits run on the deployed state land on the rule with the magic bit-identical; T4b: byte-equal to compactc 0.34.0's generated initialState for one field of every type, a curve point inside each generic evm_flow wrapper's record, the adts contract and the 1-, 16- and 226-field magic-first probes); header_small_differential (a HEADED block's state, the standard's magic included, byte-equal to compactc's generated initialState for its magic-first twin at 4 and 14 own fields, and to compactc's whole ContractState with its operations); the vault models (erc20_vault_spec, erc20_vault_pending_spec). Off-chain only: no circuit links it | a deploy state whose skeleton or defaults differ from compactc's, so a circuit reads or writes a slot that is not there, or a reader finds the wrong first leaf; a `set` that lets the magic, or a slot's shape, be replaced; a second magic, or none in a headed block |
| `minocrab-std/src/v3/discriminator.rs` | 159 | v3_discriminator (every shape the rule reads: a standard's magic at [0] and [0, 0], a Bytes<32> written first at [0], [0, 0] and [0, 0, 0]; every shape it refuses: a Counter, Map, Null, Bytes<16>, multi-atom, field-atom, EMPTY List or Merkle-tree first leaf (a pushed List<Bytes<32>> reads its head, and a hand-pathed circuit rewrites a magic: both pinned through the VM as what the rule cannot tell), an empty or non-Array root, a nest deeper than three, a Cell that does not fit its alignment; the placeholder stored trimmed and read back padded; implements true and false, and a zero-magic `implements` a compile error (doctest on v3::LedgerHeader)); ledger_deploy T5 (the serialized ContractState of every SC-003 block as the ledger holds it, and after every T4 circuit; compactc 0.34.0's own serialized magic-first states at 1, 16 and 226 fields; compactc's adts with a Counter first; bytes that are not one ContractState: every truncation, trailing bytes, another tag); header_small_differential (compactc's whole serialized ContractState, its own operations included; the 15-own-field twins, magic at [0] and [0, 0]). Off-chain only: no circuit links it | a reader that returns a leaf other than the first, pads a stored atom differently from how the ledger trims a Bytes<32>, or reads bytes that are not one ContractState of this ledger version |
...
| `minocrab-std/src/v3/stream.rs` | 409 | v3_stream (insert / take / combine / sequence on the on-chain VM, both layouts); evm_outbox (the contention audit); fee_ema (flushes of one to four); the two compile-fail doctests | a stream operation reading the shared accumulator where the design says it must not |
...
| `minocrab-macros/src/lib.rs` | 345 | **READING** | nothing beyond wrappers (the expansions are the modules below) |
...
| `minocrab-macros/src/ledger.rs` | 624 | the derive's unit tests; in_block pinned against `batch` for every block size (minocrab-std); the sixteen-field compactc probe; the headed layout through real blocks (v3_header, ledger_header) and the ZKIR dump (every block without a standard byte-identical to the base); `initial_state()`: the derive's unit tests (thinness, every field contributed in order) and the states it builds in v3_state and ledger_deploy | a ledger field laid out at a path compactc would not use, or a block given a headed layout it did not ask for, or a block whose `initial_state()` leaves a field out or keeps a state whose magics do not match its standards |
| `minocrab-macros/src/ledger_header.rs` | 406 | the derive's unit tests (expansion text, thinness, the fifteen-field bound, the rejected shapes, `r#magic` included); v3_header and ledger_header (the expansion's paths and the circuits built on them); the FR-005 compile-fail doctests; the standard's `InitialState` (the magic, sealed, then its fields): the unit test and v3_state / ledger_deploy (the magic at [0] / [0, 0], bit-identical through every circuit) | a header field placed at an entry other than [0, i + 1], or a compile-time check the expansion forgets to emit |
| `minocrab-macros/src/ledger_repr.rs` | 188 | v3_ledger `derived_repr` (atoms and limb round trip); the erc20_vault_pending lineage's slots; `default_stored` in field order: the unit test and ledger_deploy T4b's struct cell | an environment's limbs split at the wrong boundaries on read-back |
...
31139 lines in the seven crates; 11975 of them in files whose warrant is READING in whole or in part (the rows in bold).
<!-- GENERATED END -->
